...
test('appendUsage 写入 usage.jsonl；loadDays 从 days.json 恢复', async () => {
  ...
  try {
    ...
    const usageRecords = await store.loadUsage()
    assert.equal(usageRecords.length, 2)
    assert.equal(usageRecords[0]?.ts, 1000)
    ...
    const usage2 = await store2.loadUsage()
    assert.equal(usage2.length, 2)
    assert.equal(usage2[0]?.ts, 1000)
  } finally {
